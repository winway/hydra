...
def _mongo_dict_to_str(d):
    return ("%s:%d/%s/%s" % (d['srchost'], d['srcport'], d['srcdb'], d['srccol']),
            "%s:%d/%s/%s" % (d['desthost'], d['destport'], d['destdb'], d['destcol']))
...
class CopyStateDB(object):
    ...
    def add_manifest(self, manifest):
        ...
        """
        source_str, dest_str = _mongo_dict_to_str(manifest)
        with self._conn:
            cursor = self._conn.cursor()
            query  = "INSERT OR IGNORE INTO "+self.STATE_TABLE+" "
            query += "(source, dest, updated_at, state, oplog_ts) VALUES (?, ?, ?, ?, ?) "
            cursor.execute(query,
                           (source_str, dest_str, time.time(), self.STATE_WAITING_FOR_INDICES, None))
    ...
    def update_state(self, manifest, state):
        source_str, dest_str = _mongo_dict_to_str(manifest)
        query  = "UPDATE "+self.STATE_TABLE+" "
        query += "SET state = ? "
        query += "WHERE source = ? AND dest = ?"
